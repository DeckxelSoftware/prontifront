...
import {FORM_SALDO_CUENTA} from "../../form/form-saldo-cuenta";
import {TipoCuentaEnum} from "../../../../enums/tipo-cuenta.enum";
import {ModalCrearSaldoCuentaComponent} from "../modal-crear-saldo-cuenta/modal-crear-saldo-cuenta.component";

@Component({
  selector: 'app-saldo-cuenta-tabla',
  templateUrl: './saldo-cuenta-tabla.component.html',
  styleUrls: ['./saldo-cuenta-tabla.component.scss']
})
export class SaldoCuentaTablaComponent extends AbstractTable<CuentaContableResponseDto, CuentaContableFindDto>
  ...
  calcularSaldo(tipoCuenta: TipoCuentaEnum, debito: number, credito: number) {
  ...
  openDialog(formFields: FormField[], arrayAccordeon: MatStepperArray[] = []): void {
    ...
    const dialogRef = this.dialog.open(ModalCrearSaldoCuentaComponent, {
    ...
    this.httpPeriodoContableService
      ...
      .then(data => {
        ...
        const arregloDatos = data[0].map((a: any) => {
          a.detalle = a.anio + '-' + a.fechaInicio.split('T')[0] + '- ' + a.fechaFin.split('T')[0];
          return a;
        });
